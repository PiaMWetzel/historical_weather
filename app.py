...
def get_plot(todays_temps):
    ...
    get_plot1(x, tmax_all, f"Temperatures on {date} in Los Angeles since 1921 (Day)", 'static/images/highs.png')
    get_plot1(x, tmin_all, f"Temperatures on {date} in Los Angeles since 1921 (Night)",'static/images/lows.png')


    


def get_temp_comparison(low, high, temp_data):
...
def  fill_dict():
    temp_data = {month + " " + str(day):[] for month in days_in_month.keys() for day in range(1,days_in_month[month]+1)}
    return temp_data
...
def get_high_min_max(todays_temps):
    ...
    high_perc = round(100-100*(len([t for t in tmax_all if t > temperature_today.get_high()])/(len(tmax_all)-1)),2)
    max_date = [list(v.keys())[0] for v in todays_temps if list(v.values())[0]['tmax'] == max_high] 
    min_date = [list(v.keys())[0] for v in todays_temps if list(v.values())[0]['tmax'] == min_high] 
...
def get_low_min_max(todays_temps):
    ...
    low_perc = round(100-100*(len([t for t in tmin_all if t > temperature_today.get_low()])/(len(tmin_all)-1)),2)
    max_date =  [list(v.keys())[0] for v in todays_temps if list(v.values())[0]['tmin'] == max_low] 
    min_date =  [list(v.keys())[0] for v in todays_temps if list(v.values())[0]['tmin'] == min_low] 
...
def add_temperature_to_file():
    ...
    df2 = pd.DataFrame([["NA", "Downtown USC", date,high,low]], columns = ["STATION","NAME","DATE","TMAX","TMIN"])
...
def start():
    ...
    for i in range(len(la_temps["DATE"])):
        ...
        temp_data[months[int(month)-1] + " " + str(int(day))].append({year:{"tmax":tmax, "tmin":tmin}})
...
def st():
    ...
    max_high, min_high, high_perc, max_high_date, min_high_date = get_high_min_max(temp_data[date])
    max_low, min_low, low_perc, max_low_date, min_low_date = get_low_min_max(temp_data[date])
    past_temp_info = { "current_date":date,
    ...
    return render_template("st.html", high=high,low=low, icon=icon, detailed_forecast=detailed_forecast, past_temp_info=past_temp_info)
